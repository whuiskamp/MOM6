...
subroutine extractFluxes1d(G, GV, US, fluxes, optics, nsw, j, dt, &
  ...
  do i=is,ie
    ...
    write(0,*) 'frunoff at i=1,j=7 =',dt*scale*fluxes%frunoff(1,7)
    write(0,*) 'lprec at i=1,j=7 =',dt*scale*fluxes%lprec(1,7)
    write(0,*) 'fprec at i=1,j=7 =',dt*scale*fluxes%fprec(1,7)
    write(0,*) 'vprec at i=1,j=7 =',dt*scale*fluxes%vprec(1,7)
    write(0,*) 'evap at i=1,j=7 =',dt*scale*fluxes%evap(1,7)
    write(0,*) 'melt at i=1,j=7 =',dt*scale*fluxes%seaice_melt(1,7)
    ...
    !write(0,*) 'basal melt at i=10,j=10 =',dt*scale*fluxes%basal_melt(10,10)
...
subroutine register_forcing_type_diags(Time, diag, US, use_temperature, handles, use_berg_fluxes, use_waves, use_cfcs, PIK_basal)
  ...
  logical, optional,   intent(in)    :: PIK_basal       !< If true, correctly describe surface fluxes
...
! PIK_basal - pretty sure we don't want to add the basal mass flux here, but keep in mind.
...
subroutine forcing_diagnostics(fluxes_in, sfc_state, G_in, US, time_end, diag, handles)
  ...
  ! if (query_averaging_enabled(diag)) then
    ...
    !PIK_basal
    if ((handles%id_heat_content_basal > 0) .and. associated(fluxes%heat_content_basal))      &
      call post_data(handles%id_heat_content_basal, fluxes%heat_content_basal, diag)
    if ((handles%id_total_heat_content_basal > 0) .and. associated(fluxes%heat_content_basal)) then
      total_transport = global_area_integral(fluxes%heat_content_basal, G, scale=US%QRZ_T_to_W_m2)
      call post_data(handles%id_total_heat_content_basal, total_transport, diag)
    endif
